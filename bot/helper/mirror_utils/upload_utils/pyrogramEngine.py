...
from bot.helper.ext_utils.bot_utils import get_readable_file_size, is_telegram_link, is_url, sync_to_async, download_image_url, get_final_poster_url
from bot.helper.ext_utils.leech_utils import get_audio_thumb, get_media_info, get_document_type, take_ss, get_ss, get_mediainfo_link, format_filename, extract_media_info
...
class TgUploader:
    def __init__(self, name=None, path=None, listener=None):
        ...
        self.__thumb = f"/usr/src/Thumbnails/{listener.message.from_user.id}.jpg"
        self.__path = path
    ...
    async def __user_settings(self):
        ...
        self.__auto_poster = user_dict['autothumbnail'] if 'autothumbnail' in user_dict else config_dict.get('AUTO_THUMBNAIL', False)
    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            #-------------- Auto Thumbnails ----------------#
            if thumb is None:
                if self.__auto_poster:
                    try:
                        poster_url = await get_final_poster_url(file)
                        LOGGER.info(f"🎯 Poster URL for '{file}': {poster_url}")

                        if poster_url:
                            thumb = await self.get_custom_thumb(poster_url)
                        else:
                            LOGGER.warning(f"⚠️ No valid poster found for '{file}'")
                    except Exception as e:
                        LOGGER.exception(f"❌ Poster generation failed: {e}")
                else:
                    LOGGER.info("⛔️ Poster generation skipped (AUTO_THUMBNAIL is disabled).")

            # user_id = self.__listener.message.from_user.id
            # user_dict = user_data.get(user_id, {})
            # if user_dict.get('autothumbnail') or config_dict['AUTO_THUMBNAIL']:
            #     # if thumb is None and is_video:
            #     if thumb is None:
            #         movie_name, season, episode, year, part, volume, resolution = await extract_media_info(file)
            #         if movie_name:
            #             LOGGER.info(f"Attempting to fetch TMDb thumbnail for: {movie_name} ({year if year else 'Unknown Year'})")
            #             poster_url = await get_final_poster_url(file)
            #             if poster_url:
            #                 thumb = await self.get_custom_thumb(poster_url)
            #             else:
            #                 LOGGER.warning(f" No poster found for '{file}', cannot generate thumbnail.")
