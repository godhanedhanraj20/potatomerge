#!/usr/bin/env python3
import platform, re, logging, aiohttp, hashlib, asyncio, urllib.parse
...
class MirrorStatus:
    ...
    STATUS_AUDIOREMOVE = "Removing Audio"
    STATUS_AUDIOCHANGE = "Changing Audio"
    STATUS_INTROSUB    = "Intro Sub"
    STATUS_TRIM        = "Trim"
...
def get_all_versions():
    ...
    try:
        ...
        vr = ''
    try:
        vpy = distribution('pyrogram').version
    except PackageNotFoundError:
        try:
            vpy = distribution('pyrofork').version
        except PackageNotFoundError:
            vpy = "2.xx.xx"
    bot_cache['eng_versions'] = {'p7zip':vp, 'ffmpeg': vf, 'rclone': vr,
                                    'aria': aria2.client.get_version()['version'],
                                    'aiohttp': distribution('aiohttp').version,
                                    'gapi': distribution('google-api-python-client').version,
                                    'mega': MegaApi('test').getVersion(),
                                    'qbit': get_client().app.version,
                                    'pyro': vpy,
                                    'ytdlp': distribution('yt-dlp').version}
...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_METADATA]:
            ...
            status_display = getattr(download, 'status_msg', None) or download.status()
            msg += BotTheme('STATUS', Status=status_display, Url=msg_link)
...
def is_share_link(url):
    return bool(re_match(r'https?:\/\/.+\.gdtot\.\S+|https?:\/\/(.+\.filepress|filebee|appdrive|www.jiodrive)\.\S+', url))
...
async def get_ott_poster_link(title: str, season: str = None, year: str = None):
    try:
        params = {"title": title}
        if season:
        ...
        poster_url = await search_poster_with_params(params, title)
        if poster_url:
            return poster_url
            
        if (season or year) and not poster_url:
            logging.info(f"🔍 No poster found with full params, trying with just title: '{title}'")
            simple_params = {"title": title}
            return await search_poster_with_params(simple_params, title)
            
        return None
    except Exception as e:
        logging.exception(f"❌ Error fetching poster for '{title}': {e}")
    return None

async def search_poster_with_params(params, title):
    query_string = urllib.parse.urlencode(params)
    url = f"https://raj-poster.vercel.app/search?{query_string}"
    logging.info(f"Poster API URL: {url}")
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        logging.info(f"No valid landscape poster for params: {params}")
                        return None
                    elif resp.status >= 500:
                        logging.warning(f"Server error {resp.status} (attempt {attempt+1}/3)")
                        await asyncio.sleep(2)
                    else:
                        logging.warning(f"Poster API returned status {resp.status} for '{title}'")
                        return None
        except asyncio.TimeoutError:
            logging.warning(f"Timeout on attempt {attempt+1}/3 for poster API")
        except aiohttp.ClientError as e:
            logging.warning(f"Client error on attempt {attempt+1}/3: {e}")
        await asyncio.sleep(1)
    
    return None
    
async def get_tmdb_poster_link(title: str, year: str = None):
    try:
        params = {"title": title}
        if year:
            params["year"] = year
        query_string = urllib.parse.urlencode(params)
        url = f"https://poster-tmdb.spidy.eu.org/api/v1/get?{query_string}"
        logging.info(f"TMDb Poster API URL: {url}")
        timeout = aiohttp.ClientTimeout(total=10)
        for attempt in range(3):
            try:
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(url) as resp:
                        if resp.status == 200:
                            ...
                            return data.get("landscape")
                        elif resp.status >= 500:
                            logging.warning(f"Server error (status {resp.status}) on attempt {attempt+1}/3")
                            await asyncio.sleep(2)
                        else:
                            logging.warning(f"TMDb API returned status {resp.status} for '{title}'")
                            return None
            except asyncio.TimeoutError:
                logging.warning(f"Timeout on attempt {attempt+1}/3 for TMDb API")
            except aiohttp.ClientError as e:
                logging.warning(f"Client error on attempt {attempt+1}/3: {e}")
            await asyncio.sleep(1)
    except Exception as e:
        logging.exception(f"TMDb Poster API error for '{title}': {e}")
    return None

async def get_final_poster_url(raw_filename: str):
    from bot.helper.ext_utils.leech_utils import extract_media_info
    name, season, episode, year, part, volume, resolution = await extract_media_info(raw_filename)
    season_str = f"Season {season}" if season else None
    logging.info(f"Poster Search Title: {name}")
    if season:
        logging.info(f"Season Extracted: {season_str}")
    if year:
        logging.info(f"Year Extracted: {year}")
    poster_url = await get_ott_poster_link(name, season_str, year)
    if poster_url:
        logging.info("Poster found via OTT Poster API")
        return poster_url
    logging.info("Trying fallback via TMDb Poster API...")
    poster_url = await get_tmdb_poster_link(name, year)
    if poster_url:
        logging.info("Poster found via TMDb Poster API")
        return poster_url
    logging.warning("No poster found.")
    return None
    
def generate_short_filename(url: str, ext: str = ".jpg") -> str:
    short_hash = hashlib.sha1(url.encode()).hexdigest()[:8]
    return f"poster_{short_hash}{ext}"
...
async def download_image_url(url: str) -> str:
    ...
    ext = '.' + filename_part.split('.')[-1] if '.' in filename_part else ".jpg"

    filename = generate_short_filename(url, ext)
    des_dir = ospath.join(path, filename)
            ...
            async with aioClientSession() as session:
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        LOGGER.info(f"✅ Poster downloaded on attempt {attempt}: {filename}")
                        return des_dir
                    else:
                        LOGGER.warning(f"⚠️ Attempt {attempt}: Failed to download image from {url} (status {response.status})")
        except Exception as e:
            LOGGER.warning(f"⚠️ Attempt {attempt}: Exception while downloading image: {e}")

        await asyncio.sleep(1)

    LOGGER.error(f"❌ All attempts failed to download image from {url}")
    return None
    ...
    try:
        bot_cmds = [
            ...
            BotCommand(
                ...
                f'or /{BotCommands.BypassCommand[1]} Bypass Supported Shortners',
            ),
            BotCommand(
                BotCommands.TelegraphCommand,
                'Get Any Photo Link 🔗',
            ),
            BotCommand(
                BotCommands.InfoCommand,
                'Get User Info',
            ),
            BotCommand(
                BotCommands.StickeridCommand,
                'Get the sticker id of a sticker',
            ),
            BotCommand(
                BotCommands.ScrapingCommand,
                'Scraping for few movie sites',
            ),
            BotCommand(
                BotCommands.BypassCommand[0],
                f"or /{BotCommands.BypassCommand[1]} Bypass Shorten Links",
            ),
            BotCommand(
