...
class DbManger:
    ...
    async def db_load(self):
        ...
        if await self.__db.users[bot_id].find_one():
            ...
            async for row in rows:
                uid = row['_id']
                del row['_id']
                thumb_path = f'/usr/src/Thumbnails/{uid}.jpg'
                rclone_path = f'/usr/src/wcl/{uid}.conf'
                wm_path = f'/usr/src/wm/{uid}.png'
                if row.get('thumb'):
                    if not await aiopath.exists('/usr/src/Thumbnails'):
                        await makedirs('/usr/src/Thumbnails')
                    async with aiopen(thumb_path, 'wb+') as f:
                        await f.write(row['thumb'])
                    row['thumb'] = thumb_path
                if row.get('rclone'):
                    if not await aiopath.exists('/usr/src/wcl'):
                        await makedirs('/usr/src/wcl')
                    async with aiopen(rclone_path, 'wb+') as f:
                        await f.write(row['rclone'])
                    row['rclone'] = rclone_path
                if row.get('watermark'):
                    await makedirs('/usr/src/wm', exist_ok=True)
    ...
    async def update_user_doc(self, user_id, key, path=''):
        ...
        if path:
            ...
            doc_bin = ''
            await self.__db.users[bot_id].update_one({'_id': user_id}, {'$unset': {key: doc_bin}}, upsert=False)
        self.__conn.close
