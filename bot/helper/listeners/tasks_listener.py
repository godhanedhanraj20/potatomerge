...
from bot.helper.ext_utils.bot_utils import extra_btns, sync_to_async, get_readable_file_size, get_readable_time, is_mega_link, is_gdrive_link, isMkv
from bot.helper.ext_utils.fs_utils import Watermark, get_base_name, get_path_size, clean_download, clean_target, \
    is_first_archive_split, is_archive, is_archive_split, join_files, edit_metadata, add_attachment, edit_audioremove, edit_audiochange, intro_subedit, trim_video
...
from bot.helper.mirror_utils.status_utils.audioremove_status import AudioRemoveStatus
from bot.helper.mirror_utils.status_utils.audiochange_status import AudioChangeStatus
from bot.helper.mirror_utils.status_utils.attachment_status import AttachmentStatus
from bot.helper.mirror_utils.status_utils.introsub_status import IntroSubStatus
from bot.helper.mirror_utils.status_utils.trim_status import TrimStatus
...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        if config_dict['ENABLE_WATERMARK'] and await aiopath.exists(f'/usr/src/wm/{self.user_id}.png') and wm_position and wm_size:
        ...
        if metadata := self.user_dict.get('lmeta') or config_dict['METADATA']:
            ...
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0] and isMkv(meta_path):
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                outfile = await edit_metadata(self, base_dir, meta_path, outfile, metadata)
                if self.suproc == 'cancelled':
                    return
                ...
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        if self.suproc == 'cancelled':
                            return
                        video_file = ospath.join(dirpath, file)
                        if (await get_document_type(video_file))[0] and isMkv(video_file):
                            outfile = ospath.join(self.newDir, file)
                            outfile = await edit_metadata(self, dirpath, video_file, outfile, metadata)
        ...
        if attach := self.user_dict.get('lattach') or config_dict['ATTACHMENT_URL']:
            ...
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0] and isMkv(meta_path):
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                outfile = await add_attachment(self, base_dir, meta_path, outfile, attach)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0] and isMkv(video_file):
                            outfile = ospath.join(self.newDir, file)
                            outfile = await add_attachment(self, dirpath, video_file, outfile, attach)

        if introsub := self.user_dict.get('fextract_subtitle'):
            meta_path = up_path or dl_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = IntroSubStatus(name, size, gid, self)
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0] and isMkv(meta_path):
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await intro_subedit(self, base_dir, meta_path, outfile, introsub)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0] and isMkv(video_file):
                            outfile = ospath.join(self.newDir, file)
                            await intro_subedit(self, dirpath, video_file, outfile, introsub)
                            
        if trim := self.user_dict.get('ftrim'):
            meta_path = up_path or dl_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = TrimStatus(name, size, gid, self)
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0] and isMkv(meta_path):
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await trim_video(self, base_dir, meta_path, outfile, trim)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0] and isMkv(video_file):
                            outfile = ospath.join(self.newDir, file)
                            await trim_video(self, dirpath, video_file, outfile, trim)

        if audioremove := self.user_dict.get('faudioremove'):
            meta_path = up_path or dl_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = AudioRemoveStatus(name, size, gid, self)
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0] and isMkv(meta_path):
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await edit_audioremove(self, base_dir, meta_path, outfile, audioremove)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0] and isMkv(video_file):
                            outfile = ospath.join(self.newDir, file)
                            await edit_audioremove(self, dirpath, video_file, outfile, audioremove)

        if audiochange := self.user_dict.get('faudiochange'):
            meta_path = up_path or dl_path
            self.newDir = f'{self.dir}10000'
            await makedirs(self.newDir, exist_ok=True)
            async with download_dict_lock:
                download_dict[self.uid] = AudioChangeStatus(name, size, gid, self)
            if await aiopath.isfile(meta_path) and (await get_document_type(meta_path))[0] and isMkv(meta_path):
                base_dir, file_name = ospath.split(meta_path)
                outfile = ospath.join(self.newDir, file_name)
                await edit_audiochange(self, base_dir, meta_path, outfile, audiochange)
                if self.suproc == 'cancelled':
                    return
            elif await aiopath.isdir(meta_path):
                for dirpath, _, files in await sync_to_async(walk, meta_path):
                    for file in files:
                        ...
                        if (await get_document_type(video_file))[0] and isMkv(video_file):
                            outfile = ospath.join(self.newDir, file)
                            await edit_audiochange(self, dirpath, video_file, outfile, audiochange)
        ...
        if self.compress:
            ...
            elif self.user_dict.get('merge_vid') or config_dict['MERGE_VIDEO']:
        ...
        if not self.compress and self.user_dict.get('merge_vid') or config_dict['MERGE_VIDEO']:
