...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            ...
            'faudioremove': ['Audio Removal is the process that removing specified audio.', 'Example :\n\nDefault audio : audio : 0, audio : 1, audio : 2\n\nSpecified audio : 0,2\n\n<b>Timeout:</b> 60 sec'],
            'faudiochange': ['Audio Swaping is the process that modifies the order or Arrangement of audio', 'Example :\n\nDefault : audio : 0, audio : 1, audio 2, audio 3\n\nNew Arrangment : 2,1,3,0\n\n<b>Timeout:</b> 60 sec'],
            'watermark': ['Add watermark to the video. Send png image as document.', 'Send image watermark. \n<b>Timeout:</b> 60 sec'],
            'fextract_subtitle': ['Your channel name that should be used while adding an introductory subtitle track to the file.', 'Send Intro Subtrack\n\n<b>Timeout:</b> 60 sec'],
            'lautorename': ['Leech Autorename is the custom rename on the Leech Files Uploaded by the bot', 'Send Leech AutoRename Format. You Can Add Auto Rename Format like given below\nOffice {episode} {size}\nYou can add in any caption format using this option\n<b>Timeout:</b> 60 sec'],
            'mautorename': ['Mirror Autorename is the custom rename on the Mirror Files Uploaded by the bot', 'Send Mirror AutoRename Format. You Can Add Auto Rename Format like given below\nOffice {episode} {quality} {size}\nYou can add in any caption format using this option\n<b>Timeout:</b> 60 sec'],
            'ftrim': ['Trim video to a specific duration. Format: HH:MM:SS HH:MM:SS (start end)', 'Send trim times in format HH:MM:SS HH:MM:SS (start time and end time). \n<b>Timeout:</b> 60 sec'],
            }
fname_dict = {'rcc': 'RClone',
             ...
             'lattach': 'Attachment',
             'watermark': 'Watermark',
             'faudioremove': 'Audio Remove',
             'faudiochange': 'Audio Swap',
             'fextract_subtitle': 'Intro Subtrack',
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    buttons = ButtonMaker()
    thumbpath = f"/usr/src/Thumbnails/{user_id}.jpg"
    rclone_path = f'/usr/src/wcl/{user_id}.conf'
    wm_path = f'/usr/src/wm/{user_id}.png'
    ...
    if key is None:
        ...
        buttons.ibutton("Video Tools", f"userset {user_id} vidtool")
        if (user_dict and any(key in user_dict for key in list(fname_dict.keys())))  or await aiopath.exists(wm_path) or await aiopath.exists(thumbpath) or await aiopath.exists(rclone_path):
        ...
        merge_vid = "Enabled" if user_dict.get('merge_vid') or config_dict['MERGE_VIDEO'] else "Disabled"
        buttons.ibutton('✅ Merge Video' if merge_vid == 'Enabled' else 'Merge Video', f"userset {user_id} merge_vid", 'header')

        autothumbnail = "Enabled" if user_dict.get('autothumbnail') or config_dict['AUTO_THUMBNAIL'] else "Disabled"
        buttons.ibutton('✅ Auto Thumbnail' if autothumbnail == 'Enabled' else 'Auto Thumbnail', f"userset {user_id} autothumbnail", 'header'),

        lmeta = user_dict.get('lmeta') or config_dict['METADATA'] or 'Not Exists'
        buttons.ibutton(f"{'✅' if lmeta != 'Not Exists' else ''} Metadata Text", f"userset {user_id} lmeta")

        lattach = user_dict.get('lattach') or config_dict['ATTACHMENT_URL'] or 'Not Exists'
        buttons.ibutton(f"{'✅' if lattach != 'Not Exists' else ''} Attachment Url", f"userset {user_id} lattach")

        faudioremove = user_dict.get('faudioremove') or 'Not Exists'
        buttons.ibutton(f"{'✅' if faudioremove != 'Not Exists' else ''} Audio Remove", f"userset {user_id} faudioremove")
                
        faudiochange = user_dict.get('faudiochange') or 'Not Exists'
        buttons.ibutton(f"{'✅' if faudiochange != 'Not Exists' else ''} Audio Swap", f"userset {user_id} faudiochange")

        fextract_subtitle = user_dict.get('fextract_subtitle') or 'Not Exists'
        buttons.ibutton(f"{'✅' if fextract_subtitle != 'Not Exists' else ''} Intro Subtrack", f"userset {user_id} fextract_subtitle")

        watermark = 'Enabled' if await aiopath.exists(wm_path) else 'Not Exists'
        buttons.ibutton(f"{'✅' if watermark != 'Not Exists' else ''} Leech Watermark", f"userset {user_id} watermark")

        ftrim = user_dict.get('ftrim') or 'Not Exists'
        buttons.ibutton(f"{'✅' if ftrim != 'Not Exists' else ''} Video Trim", f"userset {user_id} ftrim")

        text = BotTheme('VIDTOOL', NAME=name, MERGE_MOD=merge_vid, AUTO_THUMB=autothumbnail, LMETA=escape(lmeta), LATTACH=escape(lattach), WATERMARK=escape(watermark), AUDIOREMOVE=faudioremove, FAUDIOCHANGE=faudiochange, FEXSUBTRACK=fextract_subtitle, FTRIM=ftrim)
        ...
        mautorename = 'Not Exists' if (val := user_dict.get('mautorename', '')) == '' else val
        ...
        lautorename = 'Not Exists' if (val := user_dict.get('lautorename', '')) == '' else val
        ...
        text = BotTheme('LEECH', NAME=name, DL=f"{dailyll} / {dailytlle}",
                ...
                LCAPTION=escape(lcaption), LPREFIX=escape(lprefix), LAUTORENAME=escape(lautorename),
        ...
        image = wm_path
        text = f"㊂ Select Position for Watermark:"
        ...
        image = wm_path
        text = f"㊂ Select Size for Watermark:"
        ...
        elif key in ['lprefix', 'lremname', 'lsuffix', 'lcaption', 'ldump', 'lautorename']:
        ...
        elif key in ['lmeta', 'lattach', 'faudioremove', 'faudiochange', 'fextract_subtitle', 'ftrim']:
            set_exist = 'Not Exists' if (val:=user_dict.get(key, '')) == '' else val
            text += f"➲ <b>L/M Filename {fname_dict[key]} :</b> {set_exist}\n\n"
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'yt_opt', 'lautorename'] and reply_to.text:
                return await set_custom(client, reply_to, msg, set_arg, True)
            elif set_arg in ['lmeta', 'lattach', 'faudioremove', 'faudiochange', 'fextract_subtitle', 'ftrim'] and reply_to.text:
                return await set_custom1(client, reply_to, msg, set_arg, True)
            elif set_arg == 'thumb' and reply_to.media:
                return await set_thumb(client, reply_to, msg, set_arg, True)
            elif set_arg == 'watermark' and reply_to.media:
                return await set_watermark(client, reply_to, msg, set_arg, True)
...
async def set_custom1(client, message, pre_event, key, direct=False):
    user_id = message.from_user.id
    handler_dict[user_id] = False
    value = message.text
    n_key = key
...
async def set_watermark(client, message, pre_event, key, direct=False):
    ...
    await deleteMessage(message)
    await update_user_settings(pre_event, key, 'vidtool', msg=message, sdirect=direct)
    if DATABASE_URL:
...
async def edit_user_settings(client, query):
    ...
    thumb_path = f'/usr/src/Thumbnails/{user_id}.jpg'
    rclone_path = f'/usr/src/wcl/{user_id}.conf'
    wm_path = f'/usr/src/wm/{user_id}.png'
    user_dict = user_data.get(user_id, {})
    if user_id != int(data[1]):
        await query.answer("Not Yours!", show_alert=True)
    elif data[2] in ['universal', 'mirror', 'leech', 'vidtool']:
        await query.answer()
    ...
    elif data[2] == 'vwatermark':
        handler_dict[user_id] = False
        await query.answer()
        buttons = ButtonMaker()
        buttons.ibutton('Cʟᴏsᴇ', f'wzmlx {user_id} close')
        await sendMessage(message, from_user.mention, buttons.build_menu(1), wm_path)
        await update_user_settings(query, 'watermark', 'vidtool')
    ...
    elif data[2] == 'watermark':
        if not config_dict['ENABLE_WATERMARK']:
            return await query.answer("Turn on Watermark first in bsetting!", show_alert=True)
        await query.answer()
        edit_mode = len(data) == 4
        await update_user_settings(query, data[2], 'vidtool', edit_mode)
        if not edit_mode: return
        func, is_doc = (set_watermark, True)
        pfunc = partial(func, pre_event=query, key=data[2])
        rfunc = partial(update_user_settings, query, data[2], 'vidtool')
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['autothumbnail']:
        handler_dict[user_id] = False
        if config_dict['AUTO_THUMBNAIL']:
            return await query.answer("Auto Thumbnail Force Enabled !", show_alert=True)
        await query.answer()
        update_user_ldata(user_id, data[2], not user_dict.get(data[2], False))
        await update_user_settings(query, 'vidtool')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['merge_vid']:
        handler_dict[user_id] = False
        if config_dict['MERGE_VIDEO']:
            return await query.answer("Merge Video Force Enabled !", show_alert=True)
        await query.answer()
        update_user_ldata(user_id, data[2], not user_dict.get(data[2], False))
        await update_user_settings(query, 'vidtool')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['lmeta', 'lattach', 'faudioremove', 'faudiochange', 'fextract_subtitle', 'ftrim']:
        handler_dict[user_id] = False
        if data[2] == 'lmeta' and config_dict['METADATA']:
            return await query.answer("Metadata Force Applied !", show_alert=True)        
        if data[2] == 'lattach' and config_dict['ATTACHMENT_URL']:
            return await query.answer("Attachment Url Force Applied !", show_alert=True)
        await query.answer()
        edit_mode = len(data) == 4
        return_key = 'vidtool'
        await update_user_settings(query, data[2], return_key, edit_mode)
        if not edit_mode: return
        pfunc = partial(set_custom1, pre_event=query, key=data[2])
        rfunc = partial(update_user_settings, query, data[2], return_key)
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlmeta', 'dlattach', 'dfaudioremove', 'dfaudiochange', 'dfextract_subtitle', 'dftrim']:
        handler_dict[user_id] = False
        await query.answer()
        update_user_ldata(user_id, data[2][1:])
        await update_user_settings(query, data[2][1:], 'vidtool')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'mprefix', 'msuffix', 'mremname', 'lautorename', 'mautorename']:
        handler_dict[user_id] = False
        await query.answer()
        edit_mode = len(data) == 4
        ...
        rfunc = partial(update_user_settings, query, data[2], return_key)
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlprefix', 'dlsuffix', 'dlremname', 'dlcaption', 'dldump', 'dlautorename']:
        handler_dict[user_id] = False
        await query.answer()
        ...
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['dmprefix', 'dmsuffix', 'dmremname', 'duser_tds', 'dmautorename']:
        handler_dict[user_id] = False
        await query.answer()
...
bot.add_handler(MessageHandler(send_users_settings, filters=command(
    BotCommands.UsersCommand) & CustomFilters.owner))
