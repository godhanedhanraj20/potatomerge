...
async def rcloneNode(client, message, link, dst_path, rcf, tag):
    ...
    if dst_path.startswith('mrcc:'):
        if config_path != f'/usr/src/wcl/{message.from_user.id}.conf':
...
async def clone(client, message):
    ...
    if is_rclone_path(link):
        if not await aiopath.exists('/usr/src/wcl.conf') and not await aiopath.exists(f'wcl/{message.from_user.id}.conf'):
