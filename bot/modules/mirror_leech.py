...
from bot import bot, user, DOWNLOAD_DIR, LOGGER, config_dict, bot_name, categories_dict, user_data
...
async def _mirror_leech(client, message, isQbit=False, isLeech=False, sameDir=None, bulk=[]):
    ...
    if link and is_telegram_link(link):
        try:
                result = await get_tg_link_message(link)
    ...
    if not isLeech:
        ...
        if up == 'gd' and not config_dict['GDRIVE_ID'] and not drive_id:
            ...
            if up.startswith('mrcc:'):
                config_path = f'/usr/src/wcl/{message.from_user.id}.conf'
            else:
    ...
    if user_id != int(data[1]):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post('https://api.pastes.dev/post', data={'content': logFile, 'Content-Type': 'text/log', 'User-Agent':'ios/20.1'}) as response:
                    response.raise_for_status() 
                    resp = await response.json()
            btn = ButtonMaker()
            btn.ubutton('📨 Web Paste (SB)', f"https://pastes.dev/{resp['key']}")
