...
def mega_relogin_loop():
    try:
        ...
        while True:
            if api.isLoggedIn():
                api.logout()
                # log_info('MEGA logout done')            
            sleep(5)           
            api.login(MEGA_EMAIL, MEGA_PASSWORD)            
            while not api.isLoggedIn():
                sleep(1)               
            # log_info('MEGA login done')           
            sleep(1800)
            # log_info('Starting MEGA relogin cycle')
    except Exception as e:
        log_error(f"Error in MEGA relogin loop: {e}")

if MEGA_EMAIL and MEGA_PASSWORD:
    Thread(target=mega_relogin_loop, daemon=True).start()
    # log_info('MEGA relogin thread started')
